    ...
    public class CompAmmoUser : CompRangedGizmoGiver
    {
        ...
        public void TryStartReload()
        {
            ...
            if (UseAmmo)
            {
                ...
                if (gunDrawExt != null && gunDrawExt.DropCasingWhenReload && CompEquippable?.PrimaryVerb is Verb_ShootCE verbCE)
                {
                    Log.Message(shotsFiredBeforeReload.ToString());
                    for (int i = 0; i < shotsFiredBeforeReload; i++)
                    {
                        verbCE.DropCasing(true);
                    }
                }
